@extends('dashboard.layouts.app')

@section('content')

    <!-- ============================================================== -->
    <!-- Bread crumb and right sidebar toggle -->
    <!-- ============================================================== -->
    <div class="page-breadcrumb bg-white">
        <div class="row">
            <div class="col-lg-3 col-md-4 col-xs-12 align-self-center">
                <h5 class="font-medium text-uppercase mb-0">@lang('admin.orders')</h5>
            </div>
            <div class="col-lg-9 col-md-8 col-xs-12 align-self-center">
                <nav aria-label="breadcrumb" class="mt-2 float-md-right float-left">
                    <ol class="breadcrumb mb-0 justify-content-end p-0 bg-white">
                        <li class="breadcrumb-item  "><a href="{{admin('')}}">@lang('admin.dashboard')</a></li>
                        <li class="breadcrumb-item active" aria-current="page"> @lang('admin.orders')</li>
                    </ol>
                </nav>
            </div>
        </div>
    </div>
    <!-- ============================================================== -->
    <!-- End Bread crumb and right sidebar toggle -->
    <!-- ============================================================== -->

    <!-- ============================================================== -->
    <!-- Container fluid  -->
    <!-- ============================================================== -->
    <div class="page-content container-fluid">
        @include('dashboard.layouts.messages')

        <div class="row">
            <div class="col-md-12">
                <div class="card">
                   
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <label for="search-in-table">
                                    @lang('admin.search')
                                    <input type="search" id="search-in-table" placeholder="@lang('admin.search')"
                                           class="form-control">
                                </label>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table id="table_export" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                <tr>
                                    <th scope="col" class="border " data-sort-name="id"
                                        data-sort-type="DESC">#id
                                        <img class="sort-type-image" src="{{asset('admin/image/sort_both.png')}}"
                                             alt="">
                                        <span class="sort-type-icon fa fa-sort-up d-none"></span>
                                    </th>
                                    <th scope="col" class="border" >
                                        @lang('admin.username')

                                    </th>
                                  
                                    <th scope="col" class="border" >
                                        @lang('admin.government')

                                    </th>
                                    <th scope="col" class="border" >
                                        @lang('admin.city')

                                    </th>
                                    <th scope="col" class="border" >
                                        @lang('admin.phone_number')

                                    </th>
                                    
                                    <th scope="col" class="border" >
                                        @lang('admin.total')

                                    </th>
                                    <th scope="col" class="border">@lang('admin.action')</th>   
                                    
                                </tr>
                                </thead>
                                <tbody id="search-data-append-tbody">
                                @if($orders->count())
                                    @foreach($orders as $index => $order)
                                        <tr>
                                            <td>{{$index + 1}}</td>
                                            <td>{{$order->User->name}}</td>
                                            <td>{{$order->government->name}}</td>
                                            <td>{{$order->City->name}}</td>
                                            <td>{{$order->phone_number}}</td>
                                            <td>{{$order->total}}</td>
                                       
                                            <td>
                                                @if($order->case == 1)
                                             @else
                                                <form action="{{route('orders.update',$order->id)}}"
                                                method="post"
                                                style="display: inline-block">
                                              @csrf
                                              @method('put')
                                              <input type="hidden" value="1" name="active">
                                              <button type="submit"
                                                      class="btn btn-success btn-sm">@lang('admin.Accept')
                                                  <i class="fa fa-accept"></i></button></form>
                                             @endif
                                            @if($order->case == 2)
                                            @else
                                            <form action="{{route('orders.update',$order->id)}}"
                                                method="post"
                                                style="display: inline-block">
                                              @csrf
                                              @method('put')
                                              <input type="hidden" value="2" name="active">
                                              <button type="submit"
                                                      class="btn btn-info btn-sm">@lang('admin.Refuse')
                                                      <i class="fa fa-edit"></i></i></button></form>
                                            @endif
                                            <a href="{{route('ordersdetails', $order->id )}}"
                                                class="btn btn-info btn-sm">@lang('admin.show') <i
                                                    class="fa fa-show"></i></a>

                                                    <form action="{{route('orders.delete',$order->id)}}"
                                                        method="post"
                                                        style="display: inline-block">
                                                      @csrf
                                                      @method('delete')
                                                      <button type="submit"
                                                              class="btn btn-danger btn-sm delete-btn">@lang('admin.delete')
                                                          <i class="fa fa-trash"></i></button>
                                                     
                                                  </form>
                                            </td>                
           
                                        </tr>
                                    @endforeach
                                    <tr class="">
                                        <td class="get-colspan-numbers">
                                            {{$orders->links()}}
                                        </td>
                                    </tr>
                                @else
                                    <tr>
                                        <td class="get-colspan-numbers" colspan="">@lang('admin.no_data_to_show')</td>
                                    </tr>
                                @endif
                                </tbody> 
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

@endsection
